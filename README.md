...
- `server.py` — servidor Python (aiohttp) que sirve la página y actúa de "sala" de emparejamiento para el online (1v1 o 2v2): agrupa hasta 4 sockets por sala y reenvía sus mensajes entre sí, no simula el juego (cada cliente controla su propio personaje).
...
- **Recompensas exclusivas por rango**: la primera vez que alcanzas cada uno de los 8 rangos, desbloqueas para siempre un color de piel y un halo flotante exclusivos de ese rango (ej. Diamond → piel Diamond + Halo Diamond). Se desbloquean solos (no se compran en la tienda) y aparecen automáticamente en CUSTOMIZE, en su propia sección "Halo de rango" — equipables de forma independiente al color/sombrero normal, y solo puede haber uno equipado a la vez. Bajar de rango después **no te los quita**.
- **Halos, a fondo**: cada rango tiene un diseño propio (no el mismo aro con otro color) — Bronze es una banda metálica sólida con un brillo lento; Silver añade un reflejo que recorre el aro; Gold tiene glow fuerte y partículas cayendo; Platinum son dos aros finos girando en direcciones opuestas más un campo de energía; Diamond es un aro facetado con destellos tipo estrella y fragmentos orbitando; Elite son 2-3 anillos flotantes independientes con ondas de energía expandiéndose; Champion es un aro grande con pequeñas puntas de corona integradas y un barrido de luz interno; y Unreal —el más exclusivo del juego— tiene aura propia, anillos de fragmentos orbitando, partículas rosas/blancas, pulsos de energía y un destello especial que dispara cada pocos segundos. El preview de cada halo en CUSTOMIZE está animado igual que en partida (vía CSS, sin costo de rendimiento) y crece un poco al pasar el mouse.
